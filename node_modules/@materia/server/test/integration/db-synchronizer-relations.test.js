...
describe('[Database synchronizer with relations]', () => {
    ...
    before(() => {
        ...
        })).then(() => app.entities.get('city').addRelation({
            type: 'belongsTo',
            field: 'id_region',
            reference: {
                entity: 'region',
        ...
        })).then(() => app.entities.get('region').addRelation({
            type: 'belongsTo',
            field: 'id_country',
            reference: {
                entity: 'country',
    ...
    describe('App', () => {
        ...
        it('app should have diffs after deleting models.json', () => {
            return fse.remove(path.join(app.path, 'server', 'models', 'country.json'))
                .then(() => fse.remove(path.join(app.path, 'server', 'models', 'region.json'))).then(() => fse.remove(path.join(app.path, 'server', 'models', 'city.json'))).then(() => fse.remove(path.join(app.path, 'server', 'models', 'test.json'))).then(() => {
                return app.stop();
            })
                .then(() => {
                return app.load();
        ...
        it('synchronizing should re-add entities and relations', () => {
            return app.synchronizer.diff()
                .then((diffs) => app.synchronizer.databaseToEntities(diffs, null)).then(() => {
                return app.entities.get('city');
            }).then((entity) => entity.fields.map(field => field.name)).should.become(['id_city', 'name', 'id_region']);
        });
...
//# sourceMappingURL=db-synchronizer-relations.test.js.map