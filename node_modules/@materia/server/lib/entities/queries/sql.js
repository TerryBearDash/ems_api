...
class SQLQuery extends query_1.Query {
    constructor(entity, id, opts) {
        super(entity, id);
        if (!opts || !opts.query) {
            throw new error_1.MateriaError('Missing required parameter "query"');
        }
        this.type = 'sql';
    ...
    run(params) {
        this.entity.app.logger.log(`${chalk_1.default.bold('(Query)')} SQL - Run ${chalk_1.default.bold(this.entity.name)}.${chalk_1.default.bold(this.id)}`);
        let resolvedParams;
        try {
        ...
        return this.handleBeforeActions(params, true)
            .then(() => this.entity.app.database.sequelize.query(this.query, {
