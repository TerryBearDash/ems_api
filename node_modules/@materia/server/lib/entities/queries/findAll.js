...
class FindAllQuery extends query_1.Query {
    ...
    discoverParam(param, type, required) {
        if (!required) {
            required = false;
        }
    ...
    run(params, options) {
        if (!options || !options.silent) {
            this.entity.app.logger.log(`${chalk_1.default.bold('(Query)')} FindAll - Run ${chalk_1.default.bold(this.entity.name)}.${chalk_1.default.bold(this.id)}`);
    ...
    _paramResolver(name, value, params, defaultValue) {
        let tmp;
        try {
            tmp = query_1.QueryParamResolver.resolve({ name: name, value: value }, params);
            if (!tmp) {
    ...
    _run(sequelizeOpts, options) {
        return this.entity.model.findAndCountAll(sequelizeOpts).then(res => {
            if (!options || !options.silent) {
                this.entity.app.logger.log(` └── ${chalk_1.default.green.bold('OK')}\n`);
            }
            let result = {
                data: res.rows,
                count: res.count
