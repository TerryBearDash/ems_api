...
const findAll_1 = require("./queries/findAll");
const findOne_1 = require("./queries/findOne");
...
const custom_1 = require("./queries/custom");
class DBEntity extends entity_1.Entity {
    constructor(app) {
        super(app, {
            findAll: findAll_1.FindAllQuery,
            findOne: findOne_1.FindOneQuery,
            create: create_1.CreateQuery,
            update: update_1.UpdateQuery,
            delete: delete_1.DeleteQuery,
            custom: custom_1.CustomQuery,
            sql: sql_1.SQLQuery
        });
        this.reservedQueries = [
    ...
    updateField(name, field, options) {
        ...
        return new Promise((accept, reject) => {
            if (!oldfield) {
                return reject(new error_1.MateriaError('This field does not exist'));
            }
            let fieldobj;
            let differ_done;
            options.differ = (done) => {
            ...
            const _field = Object.assign({}, oldfield, field);
                ...
                if (field.generateFrom) {
                    p = p.then(() => {
                        ...
                        return entityFrom.getQuery('list').run({ limit: 1 }, { raw: true }).then((list) => {
                            if (!list.data.length) {
                                const query = this.getQuery('list');
                                ...
                                return query.run({ limit: 1 }).then((from_list) => {
                                    if (from_list.data.length) {
                                        ...
                                        fieldInstance.update(Object.assign({}, fieldInstance.toJson(), {
                                            onDelete: 'NO ACTION',
                                            required: false,
                                            default: false,
                                            unique: isUnique,
    ...
    generateDefaultQueries() {
        const qg = new query_generator_1.QueryGenerator(this);
        qg.generateQueries();
    }
    loadModel() {
