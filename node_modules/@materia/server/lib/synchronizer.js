...
class Synchronizer {
    ...
    diff() {
        return this.app.database.interface.showTables().then((dbTables) => {
            const entities = this.app.entities.findAll().filter((entity) => entity instanceof db_entity_1.DBEntity);
    ...
    _compareField(dbfield, field, entity) {
        ...
        if (this.app.database && this.app.database.type === 'sqlite' &&
        ...
        if (dbfield.unique !== field.unique &&
            !dbfield.primary &&
            !field.primary &&
            !(dbfield.unique === true &&
    ...
    _constructEntitiesDiffs(entitiesDiffs) {
        let result = [];
        if (entitiesDiffs && entitiesDiffs.length) {
            const entitiesWithoutRelationsDiffs = entitiesDiffs.filter(diff => diff.redo &&
                diff.redo.type === history_1.MigrationType.CREATE_ENTITY &&
                diff.redo.value &&
                (!diff.redo.value.relations || (diff.redo.value.relations
                    && diff.redo.value.relations.length === 0)));
            const entitiesWithRelationsDiffs = entitiesDiffs.filter(diff => diff.redo &&
                diff.redo.type === history_1.MigrationType.CREATE_ENTITY &&
                diff.redo.value &&
            ...
            const otherEntitiesDiffs = entitiesDiffs.filter(diff => !diff.redo || (diff.redo &&
                diff.redo.type !== history_1.MigrationType.CREATE_ENTITY));
            result = [...result, ...otherEntitiesDiffs];
        }
        return result;
    }
    _sortCreateEntitiesDiffsWithRelations(entitiesWithRelationsDiffs, loadedDiffs) {
        let finish = true;
        entitiesWithRelationsDiffs.forEach(diff => {
            const relatedEntities = diff.redo.value.relations.map((relation) => relation.reference.entity);
