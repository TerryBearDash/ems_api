...
const sequelize_typemap = {
    ...
    'number': sequelize_1.INTEGER,
    'boolean': sequelize_1.BOOLEAN,
    'float': sequelize_1.FLOAT,
};
/**
    ...
    */
    addColumn(table, column_name, field) {
        if (field.type === 'date' && field.defaultValue === 'now()') {
            field.defaultValue = sequelize_1.NOW;
    ...
    columnToField(field) {
        ...
        for (const type of (Array.isArray(field.type) ? field.type : [field.type])) {
            ...
            if (!typemap[formattedType]) {
                throw new error_1.MateriaError('Unknown type : "' + formattedType + '"');
            }
    ...
    fieldToColumn(field) {
        ...
        if (field.type != undefined) {
            if (this.database.type == 'mysql' && (!type || type == 'text')) {
                res.type = sequelize_1.STRING;
            }
            else {
                res.type = (type && sequelize_typemap[type]) || sequelize_1.TEXT;
            }
        }
        ...
        if (field.default == '$now' && type.toLowerCase() == 'date') {
            res.default = sequelize_1.Sequelize.fn('NOW');
        }
