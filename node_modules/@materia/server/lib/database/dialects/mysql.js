...
class MysqlDialect extends abstract_1.AbstractDialect {
    ...
    define(entityName, cols, defOptions) {
        for (const colName in cols) {
            if (cols[colName]) {
                ...
                if (col && col.defaultValue && col.defaultValue === sequelize_1.NOW && col.type === 'date') {
                    col.defaultValue = sequelize_1.Sequelize.fn('NOW');
                }
            }
        }
    ...
    _getFKs(table) {
        const query = 'SELECT INFORMATION_SCHEMA.KEY_COLUMN_USAGE.CONSTRAINT_NAME as `constraint_name`,' +
            ...
            return this.getIndices(table).then((fields) => {
                ...
                for (const index of fields[constraint.field]) {
                    if (index.unique) {
                        ((i) => {
                            p = p.then(() => {
                                return this.sequelize.query(`ALTER TABLE ${table} DROP INDEX ${i.name}`);
                            });
                        })(index);
                    }
                }
