...
class PostgresDialect extends abstract_1.AbstractDialect {
    ...
    define(entityName, cols, defOptions) {
        for (const colName in cols) {
            if (cols[colName]) {
                ...
                if (col.defaultValue === sequelize_1.NOW && col.type === 'date') {
                    col.defaultValue = sequelize_1.Sequelize.literal('NOW');
                }
            }
        }
    ...
    addColumn(table, column_name, attributes) {
        if (attributes.defaultValue === sequelize_1.NOW) {
            attributes.defaultValue = sequelize_1.Sequelize.fn('NOW');
        }
    ...
    showTables() {
        ...
        return this.sequelize.getQueryInterface().showAllTables().then((tables) => {
            ...
            return Promise.all(promises).then((result) => {
                ...
                const err = new error_1.MateriaError('Error when scanning database');
                err['originalError'] = e;
                throw err;
            });
        });
                ...
                return this._getFKs(table).then((fks) => {
                    for (const fk of fks) {
                        if (fk.from == constraint.field) {
                            return this.sequelize.query(`ALTER TABLE "${table}" DROP CONSTRAINT "${fk.constraint_name}"`);
                        }
                    }
                });
            }
            return this.getIndices(table).then((fields) => {
                ...
                for (const index of fields[constraint.field]) {
                    if (index.unique) {
                        ((i) => {
                            p = p.then(() => {
                                return this.sequelize.query(`ALTER TABLE "${table}" DROP CONSTRAINT IF EXISTS "${i.name}"`).then(() => {
                                    return queryInterface.removeIndex(table, i.name);
                                });
                            });
                        })(index);
                    }
