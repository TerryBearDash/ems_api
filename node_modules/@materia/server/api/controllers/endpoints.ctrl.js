...
class EndpointsController {
    ...
    getEndpoints(req, res) {
        const endpoints = this.app.api.findAll().map(api => Object.assign({}, api.toJson(), {
    ...
    loadController(req, res) {
        ...
        try {
            ...
            if (fromAddon) {
                ...
                endpointPath = path.join(this.app.path, 'node_modules', addonName, 'server', 'controllers');
            }
            const code = fs
                .readFileSync(path.join(endpointPath, controllerName + '.ctrl.js'), 'utf-8')
                .toString();
    ...
    createCode(req, res) {
        ...
        const controller = endpoint.fromAddon
            ? endpoint.controller
                .split('/')[endpoint.controller.split('/').length - 1].replace(/(\.ctrl)?\.js$/, '')
            : endpoint.controller.replace(/(\.ctrl)?\.js$/, '');
        const basePath = endpoint.fromAddon
            ? endpoint.fromAddon.path
            : this.app.path;
        const fullpath = path.join(basePath, 'server', 'controllers', controller + '.ctrl.js');
        ...
        promise.then(() => {
            ...
            res.status(201).json({
                endpoints: endpoints_1.EndpointsLib.list(this.app),
    ...
    updateCode(req, res) {
        ...
        const controller = newEndpoint.controller.replace(/(\.ctrl)?\.js$/, '');
        ...
        promise.then(() => {
            ...
            if (newEndpoint.params) {
                params = endpoints_1.EndpointsLib.cleanParams(newEndpoint.params);
            }
            ...
            endpoints_1.EndpointsLib.list(this.app).find(e => e.method + e.url == newEndpoint.method + newEndpoint.url);
            this.app.watcher.enable();
            res.status(200).json({
                endpoints: endpoints_1.EndpointsLib.list(this.app),
                newSelectedId: newEndpoint.method +
                    newEndpoint.url,
                controllers: this.app.api.getControllers()
            });
        }).catch(err => {
    ...
    updateQuery(req, res) {
        ...
        if (newEndpoint.params &&
            ...
            newEndpoint.params = endpoints_1.EndpointsLib.cleanParams(newEndpoint.params);
        }
        ...
        res.status(200).json({
            endpoints: endpoints_1.EndpointsLib.list(this.app),
            newSelectedId: newEndpoint.method + newEndpoint.url,
            controllers: this.app.api.getControllers()
        });
    }
    remove(req, res) {
    ...
    generate(req, res) {
        ...
        this.app.watcher.disable();
        endpoints_1.EndpointsLib.generate(this.app, entity, 'get', 'list');
        endpoints_1.EndpointsLib.generate(this.app, entity, 'get', 'get');
        endpoints_1.EndpointsLib.generate(this.app, entity, 'post', 'create');
        endpoints_1.EndpointsLib.generate(this.app, entity, 'put', 'update');
        endpoints_1.EndpointsLib.generate(this.app, entity, 'delete', 'delete');
        this.app.watcher.enable();
        res.status(200).json({ endpoints: endpoints_1.EndpointsLib.list(this.app) });
    }
}
exports.EndpointsController = EndpointsController;
//# sourceMappingURL=endpoints.ctrl.js.map