...
class AddonsController {
    ...
    enable(req, res) {
        ...
        if (!pkg || !this.app.addons.get(pkg)) {
            ...
            this.app.addons.get(pkg).enable().then(() => res.status(200).send()).catch(e => res.status(500).json(e));
        }
    }
    disable(req, res) {
        const pkg = this.getPkgFromRequest(req);
        if (!pkg || !this.app.addons.get(pkg)) {
            ...
            this.app.addons.get(pkg).disable().then(() => res.status(200).send()).catch(e => res.status(500).json(e));
        }
    }
    bundle(req, res) {
        const pkg = this.getPkgFromRequest(req);
        if (!pkg || !this.app.addons.get(pkg)) {
            ...
            return fse.readFile(this.app.addons.get(pkg).getBundlePath()).then((bundle) => res.status(200).send(bundle)).catch((err) => res.status(500).send(err));
        }
    }
...
//# sourceMappingURL=addons.ctrl.js.map