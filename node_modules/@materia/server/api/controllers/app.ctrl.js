...
class AppController {
    ...
    saveServerSettings(app, settings, mode) {
        if (settings.server[mode] &&
            settings.server[mode].host &&
            settings.server[mode].host) {
            return app.config.set(settings.server[mode], mode, lib_1.ConfigType.SERVER, {
                live: false
            });
        }
    }
    saveDatabaseSettings(app, settings, mode) {
        if (settings.database && settings.database[mode]) {
            if (settings.database[mode].type &&
            ...
            return app.config.set(settings.database[mode], mode, lib_1.ConfigType.DATABASE, { live: false });
        }
    }
    saveClientSettings(app, settings) {
        if (settings.client) {
            const client = settings.client;
            if (client.packageJson) {
                app.config.set(client.packageJson.devDependencies, 'dev', lib_1.ConfigType.DEPENDENCIES);
                app.config.set(client.packageJson.dependencies, 'prod', lib_1.ConfigType.DEPENDENCIES);
                app.config.set(client.packageJson.scripts, 'dev', lib_1.ConfigType.SCRIPTS);
            ...
            if (client.www) {
                ...
                app.config.set(clientToSave, 'dev', lib_1.ConfigType.CLIENT);
            }
            else {
                app.config.delete(lib_1.ConfigType.CLIENT);
            }
        }
    }
    config(req, res) {
        ...
        const appConfig = this.app.config.get(lib_1.AppMode.DEVELOPMENT, lib_1.ConfigType.APP);
        this.app.config.set({
        ...
        }, lib_1.AppMode.DEVELOPMENT, lib_1.ConfigType.APP);
    ...
    getMinimalInfo(req, res) {
        res.status(200).json({
            ...
            addons: this.app.addons.findAll().map(addon => Object.assign({}, addon.toJson(), {
    ...
    getInfos(req, res) {
        const appConf = this.app.config.get('dev', lib_1.ConfigType.APP);
        res.status(200).json({
            id: this.app.id,
            package: this.app.package,
            name: this.app.name,
            ...
            database: {
                ...
                config: {
                    dev: this.app.config.get('dev', lib_1.ConfigType.DATABASE),
                    prod: this.app.config.get('prod', lib_1.ConfigType.DATABASE)
                }
            },
            server: {
                dev: this.app.config.get('dev', lib_1.ConfigType.SERVER),
                prod: this.app.config.get('prod', lib_1.ConfigType.SERVER)
            },
            client: this.app.config.get('dev', lib_1.ConfigType.CLIENT),
            entities: database_1.DatabaseLib.loadEntitiesJson(this.app),
