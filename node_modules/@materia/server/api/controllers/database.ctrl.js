...
class DatabaseController {
    ...
    getEntities(req, res) {
        return res.status(200).send({ entities: lib_1.DatabaseLib.loadEntitiesJson(this.app) });
    }
    ...
    removeEntity(req, res) {
        ...
        this.app.entities.remove(name, {
            ...
            this.app.watcher.enable();
            res.status(200).json({ entities: lib_1.DatabaseLib.loadEntitiesJson(this.app), endpoints: this.app.api.findAll().map(e => e.toJson()) });
        }).catch(err => {
            this.app.watcher.enable();
    ...
    renameEntity(req, res) {
        ...
        this.app.entities.rename(oldName, new_name, {
            ...
            .then(() => {
            this.app.watcher.enable();
            res.status(200).json(Object.assign({}, this.app.entities.get(new_name).toJson(), {
                name: new_name
            }));
        }).catch(err => {
            this.app.watcher.enable();
    ...
    saveField(req, res) {
        ...
        promise.then(() => {
            this.app.watcher.enable();
            return res.status(200).send(lib_1.DatabaseLib.loadEntitiesJson(this.app));
        }).catch(err => {
            this.app.watcher.enable();
    ...
    loadModel(req, res) {
        const fromAddon = req.query.fromAddon;
        const modelName = req.params.model;
        const basePath = fromAddon
            ? fromAddon
            : path.join(this.app.path, 'server', 'models', 'queries');
        fs.readFile(path.join(basePath, modelName + '.js'), 'utf-8', (err, data) => {
            if (err) {
                return res.status(500).send({ error: true, message: err.message });
            }
            if (data) {
    ...
    removeField(req, res) {
        ...
        entityInstance.removeField(field, {
            ...
            .then(() => {
            this.app.watcher.enable();
            res.status(200).json(lib_1.DatabaseLib.loadEntitiesJson(this.app));
        })
            .catch(err => {
            this.app.watcher.enable();
            res.status(500).send({ error: true, message: err.message });
    ...
    createQuery(req, res) {
        ...
        if (!entity) {
            return res.status(400).send(new lib_1.MateriaError(`Entity "${req.params.entity}" does not exists`));
        }
        this.app.watcher.disable();
        ...
        if (query.type == 'custom') {
            const basePath = entity.fromAddon
                ...
                : this.app.path;
            p = p.then(() => this.app.saveFile(path.join(basePath, 'server', 'models', 'queries', query.opts.model + '.js'), query.code, { mkdir: true }));
        }
        p.then(() => entity.addQuery(query)).then(() => {
            this.app.watcher.enable();
            res.status(200).send(lib_1.DatabaseLib.loadEntitiesJson(this.app));
        }).catch(err => {
            this.app.watcher.enable();
            res.status(500).send({ error: true, message: err.message });
        });
    ...
    runQuery(req, res) {
        ...
        const query = entityInstance.getQuery(queryId);
        if (!query) {
    ...
    addAction(req, res) {
        lib_1.DatabaseLib.generateActionId().then(id => {
    ...
    createRelation(req, res) {
        ...
        this.app.entities
            ...
            return res.status(201).json({
                entities: lib_1.DatabaseLib.loadEntitiesJson(this.app),
...
//# sourceMappingURL=database.ctrl.js.map