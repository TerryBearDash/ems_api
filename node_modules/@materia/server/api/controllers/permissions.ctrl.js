...
exports.PermissionsLib = PermissionsLib;
class PermissionsController {
    ...
    add(req, res) {
        ...
        const exists = this.app.api.permissions.findAll().findIndex(p => p.name === newPermission.name) !== -1;
        if (exists) {
        ...
        this._checkNewPermission(newPermission).then((permissionCode) => {
            if (permissionCode !== '') {
                try {
                    const file = path_1.join(this.app.path, 'server', 'permissions', newPermission.file);
    ...
    _checkNewPermission(perm) {
        return new Promise((resolve, reject) => {
            const filepath = path_1.join(this.app.path, 'server', 'permissions', `${perm.file}.js`);
    ...
    _getPermissionFilepath(permission) {
        let filename = permission.file;
        if (filename.indexOf(path_1.sep) == -1) {
            filename = path_1.join(this.app.path, 'server', 'permissions', permission.file);
        }
...
//# sourceMappingURL=permissions.ctrl.js.map