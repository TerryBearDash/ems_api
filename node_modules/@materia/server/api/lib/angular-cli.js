...
class AngularCli {
    ...
    exec(command, params, cwd) {
        return new Promise((resolve, reject) => {
            ...
            if (fs.existsSync(path_1.join(cwd, 'node_modules', '.bin', 'ng'))) {
                stream = execa(path_1.join(cwd, 'node_modules', '.bin', 'ng'), [command, ...params], {
                    cwd: cwd
                });
                ...
                reject(new Error(`@angular/cli dependency not found in ${path_1.join(cwd, 'node_modules')}`));
            }
        });
    }
    getConfig() {
        return new Promise((resolve, reject) => {
            fs.readFile(path_1.join(this.app.path, 'angular.json'), 'utf-8', (e, data) => {
                if (e) {
                    reject(e);
                }
                else {
    ...
    initNewMonopackageConfig(projectName) {
        return new Promise((resolve, reject) => {
            this.getConfig().then(() => {
                this.config.projects[projectName].root = 'client';
                this.config.projects[projectName].sourceRoot = 'client/src';
                this.config.projects[projectName].architect.build.options = Object.assign({}, this.config.projects[projectName].architect.build.options, {
                ...
                this.config.projects[projectName].architect.build.configurations = Object.assign({}, this.config.projects[projectName].architect.build.configurations, {
                    'production': {
                        'fileReplacements': [
                            {
    ...
    saveConfig() {
        return this.app.saveFile(path_1.join(this.app.path, 'angular.json'), JSON.stringify(this.config, null, 2));
    }
}
