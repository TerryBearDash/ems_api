...
class DatabaseLib {
    static generateActionId({ stringBase = 'base64', byteLength = 8 } = {}) {
        ...
        return Object.assign({}, entity.toJson(), {
            ...
            relatedEntities: entity
                ...
                return Object.assign({}, relatedEntity.toJson(), {
                    ...
                    fields: relatedEntity.fields.map(field => field.toJson()),
    ...
    static entityWidth(entity) {
        return 200;
    }
    static entityHeight(entity) {
        return 40 + 48 * entity.fields.length;
    }
    static getWorkspaceSize(entities) {
    ...
    static isAvailableSpace(entities, entity, pX, pY) {
        ...
        for (const i in entities) {
            if (entities[i].x != -1 && entities[i].y != -1) {
                if (entities[i].x > pX - DatabaseLib.entityWidth(entities[i]) &&
        ...
        if (workspaceSize.height >= workspaceSize.width) {
            ...
            for (const i in entities) {
                if (entities[i].x != -1 && entities[i].y != -1) {
                    ...
                    const pY = entities[i].y + DatabaseLib.entityHeight(entities[i]);
                    if (pY < posY &&
