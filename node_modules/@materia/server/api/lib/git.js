...
class Git {
        ...
        return this.getLogs().then(logs => {
            const history = logs.all.map(log => {
                ...
                log.full_date = `${log.date.getUTCMonth() +
                    1}/${log.date.getUTCDate()}/${log.date.getUTCFullYear()} at ${log.date.getUTCHours()}:${log.date.getUTCMinutes()}:${log.date.getUTCSeconds()}`;
    ...
    getStatusDiff(statusPath) {
        ...
        try {
            const size = fs.statSync(path_1.join(this.app.path, statusPath)).size;
            if (size > 1000000.0) {
                content = '// The content is too long to display...';
            }
            else {
                content = fs.readFileSync(path_1.join(this.app.path, statusPath), 'utf8');
            }
        }
        catch (e) { }
    ...
    unstage(statusPath) {
        ...
        return this.client
            ...
            if (e &&
                e.message &&
                e.message.match(/ambiguous argument 'HEAD': unknown revision/)) {
    ...
    getLogs() {
        ...
        return this.client.log(args).catch(e => {
            if (e &&
                e.message &&
                e.message.match(/your current branch '.*?' does not have any commits yet/)) {
